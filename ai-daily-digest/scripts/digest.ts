...
const DEFAULT_ROLE_ID = 'tech-blogger';
...
interface RoleConfig {
  ...
  scoring?: {
    ...
    relevanceLevels?: { high?: string; medium?: string; low?: string };
  };
  branding?: {
    title?: string;
    subtitle?: string;
...
async function loadRole(roleId: string): Promise<RoleConfig> {
  const scriptDir = getScriptDir();
  const rolePath = join(scriptDir, '..', 'roles', `${roleId}.json`);

  try {
    const raw = await readFile(rolePath, 'utf-8');
    const role = JSON.parse(raw) as RoleConfig;

    if (!role.id || role.id !== roleId) {
      throw new Error(`Role id "${role.id}" does not match filename "${roleId}"`);
    }
    if (!role.name || typeof role.name !== 'string') {
      throw new Error(`Role "${roleId}" is missing required field "name"`);
    }
    if (!Array.isArray(role.feeds) || role.feeds.length === 0) {
      throw new Error(`Role "${roleId}" must have at least one feed`);
    }
    for (const feed of role.feeds) {
      if (!feed.name || !feed.xmlUrl) {
        throw new Error(`Role "${roleId}" has a feed missing "name" or "xmlUrl"`);
      }
    }
    // Ensure categories always has an "other" fallback
    if (role.categories && !role.categories['other']) {
      role.categories['other'] = { emoji: '📝', label: '其他' };
    }

    return role;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new Error(`Role file not found: ${rolePath}\nRun with --list-roles to see available roles.`);
    }
    throw err;
  }
}
...
async function listRoles(): Promise<void> {
  const scriptDir = getScriptDir();
  const rolesDir = join(scriptDir, '..', 'roles');
  ...
  for (const file of jsonFiles) {
    try {
      ...
      const role = JSON.parse(raw) as RoleConfig;
      const feedCount = role.feeds?.length || 0;
      const id = (role.id || file.replace('.json', '')).padEnd(18);
      const name = (role.name || '').padEnd(14);
      console.log(`  ${id} ${name} (${feedCount} feeds)  ${role.description || ''}`);
    } catch {
      console.log(`  ${file.padEnd(18)} (invalid JSON)`);
    }
  }
  console.log('');
  console.log('Usage: bun scripts/digest.ts --role <role-id> [options]');
  console.log('Create custom roles: add a JSON file to the roles/ directory.');
}
...
const DEFAULT_CATEGORIES: Record<string, { emoji: string; label: string }> = {
...
function buildScoringPrompt(
  ...
  // Persona from role, or default
  const persona = role.scoring?.persona
    || '你是一个技术内容策展人，正在为一份面向技术爱好者的每日精选摘要筛选文章';

  // Relevance description from role, or default
  const relevanceDesc = role.scoring?.relevanceDescription
    || '对技术/编程/AI/互联网从业者的价值';

  const levels = role.scoring?.relevanceLevels;
  const relevanceDetail = levels
    ? `- 10: ${levels.high || '所有从业者都应该知道的重大事件/突破'}
- 7-9: ${levels.medium || '对大部分从业者有价值'}
- 4-6: 对特定领域有价值
- 1-3: ${levels.low || '关联不大'}`
...
- 1-3: 与技术行业关联不大`;
  ...
  // Categories from role, or default
  const categories = role.categories || DEFAULT_CATEGORIES;
  const categoryList = Object.entries(categories)
    .map(([id, meta]) => `- ${id}: ${meta.label}相关`)
    .join('\n');
  const firstCatId = Object.keys(categories)[0] || 'other';
...
async function scoreArticlesWithAI(
  ...
  const activeCategories = role.categories || DEFAULT_CATEGORIES;
  const validCategories = new Set<string>(Object.keys(activeCategories));
  
  for (let i = 0; i < batches.length; i += MAX_CONCURRENT_GEMINI) {
    const batchGroup = batches.slice(i, i + MAX_CONCURRENT_GEMINI);
    const promises = batchGroup.map(async (batch) => {
      try {
        const prompt = buildScoringPrompt(batch, role);
...
async function generateHighlights(
  ...
  const domainLabel = role.branding?.highlightsLabel || '今天技术圈';

  const prompt = `根据以下今日精选文章列表，写一段 3-5 句话的"今日看点"总结。
要求：
- 提炼出${domainLabel}的 2-3 个主要趋势或话题
- 不要逐篇列举，要做宏观归纳
- 风格简洁有力，像新闻导语
${langNote}
...
function generateDigestReport(articles: ScoredArticle[], highlights: string, stats: {
  ...
  hours: number;
  lang: string;
}, role: RoleConfig): string {
  const now = new Date();
  const dateStr = now.toISOString().split('T')[0];
  const activeCategories = role.categories || DEFAULT_CATEGORIES;
  const branding = role.branding || {};

  const title = branding.title || 'AI 博客每日精选';
  const subtitle = branding.subtitle || `来自 ${stats.totalFeeds} 个信息源`;

  let report = `# 📰 ${title} — ${dateStr}\n\n`;
  report += `> ${subtitle}，AI 精选 Top ${articles.length}\n\n`;
  ...
  if (articles.length >= 3) {
    ...
    for (let i = 0; i < Math.min(3, articles.length); i++) {
      const a = articles[i];
      const medal = ['🥇', '🥈', '🥉'][i];
      const catMeta = activeCategories[a.category] || { emoji: '📝', label: a.category };
  ...
  const pieChart = generateCategoryPieChart(articles, activeCategories);
  if (pieChart) {
    report += `### 分类分布\n\n${pieChart}\n`;
  }
  ...
  for (const [catId, catArticles] of sortedCategories) {
    const catMeta = activeCategories[catId] || { emoji: '📝', label: catId };
...
function printUsage(): never {
  console.log(`AI Daily Digest - AI-powered RSS digest with multi-role support

Usage:
  bun scripts/digest.ts [options]

Options:
  --role <id>     Role to use (default: tech-blogger). See --list-roles
  ...
  --output <path> Output file path (default: ./digest-<role>-YYYYMMDD.md)
  --help          Show this help
...
Examples:
  bun scripts/digest.ts --role tech-blogger --hours 48 --top-n 15
  bun scripts/digest.ts --role ai-pm --hours 24 --top-n 10 --lang zh
  bun scripts/digest.ts --role programmer --hours 48 --top-n 15
  bun scripts/digest.ts --role trader --hours 24 --top-n 10
  bun scripts/digest.ts --list-roles
`);
  process.exit(0);
}
...
async function main(): Promise<void> {
  ...
  if (!outputPath) {
    ...
    outputPath = join(getScriptDir(), '..', 'output', `digest-${roleId}-${dateStr}.md`);
  }

  console.log(`[digest] === AI Daily Digest ===`);
  console.log(`[digest] Role: ${role.name} (${roleId})`);
  ...
  console.log(`[digest] Step 1/5: Fetching ${role.feeds.length} RSS feeds...`);
  const allArticles = await fetchAllFeeds(role.feeds);
  ...
  const scores = await scoreArticlesWithAI(recentArticles, aiClient, role);
